...
def load_data(stock_ticker, start, end):
    df = yf.download(stock_ticker, start=start, end=end, multi_level_index=False)
    return df
...
try:
    ...
    if data.empty:
        ...
        # 4. Drop any empty/NaN values specifically from the close prices (fixing the market-closed issue)
        close_series = close_series.dropna()

        # 5. Calculate Moving Average using our clean Series
        ...
        # 7. Visualizations (Interactive Plotly Chart)
        ...
        # 8. Raw Data Table (Financial Analysts love raw numbers)
